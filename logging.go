...
var defaultLogging value.Value = value.NewSimpleValue(Logging(NewLogging()))
...
// 配置默认的Logging、Factory value(非线程安全)
// 可以配置为线程安全(value.AtomicValue)的和非线程安全的(value.SimpleValue)
...
var once = sync.Once{}
// 配置默认的Logging、Factory value(线程安全且只能初始化一次)
// 可以配置为线程安全(value.AtomicValue)的和非线程安全的(value.SimpleValue)
